...
class Decoder(nn.Module):
    def __init__(self, n_feat, kernel_size, reduction, act, bias, scale_unetfeats):
        ...
        self.decoder_level1 = [CAB(n_feat * 2, kernel_size, reduction, bias=bias, act=act) for _ in range(2)]
        self.decoder_level2 = [CAB((n_feat + scale_unetfeats) * 2, kernel_size, reduction, bias=bias, act=act) for _ in range(2)]
        self.decoder_level3 = [CAB((n_feat + (scale_unetfeats * 2)) * 2, kernel_size, reduction, bias=bias, act=act) for _ in range(2)]
    ...
    def forward(self, encoder_outs):
        enc1, enc2, enc3 = encoder_outs
        
        dec3 = self.decoder_level3(torch.cat([enc3, enc3], dim=1))
        x = self.up32(dec3, self.skip_attn2(enc2))
        dec2 = self.decoder_level2(torch.cat([x, enc2], dim=1))
        
        x = self.up21(dec2, self.skip_attn1(enc1))
        dec1 = self.decoder_level1(torch.cat([x, enc1], dim=1))
